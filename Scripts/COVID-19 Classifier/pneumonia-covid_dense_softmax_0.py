...
def RandomResizedCrop(img, size=(224, 224), scale=(0.5, 1.0), ratio=(3./ 4., 4./3.), occlusion=False, interpolation=cv2.INTER_LINEAR):
    ...
    cond = random.random()
    if  cond > 0.5:
        offset = random.randint(height//5, height//4)
        img[:offset] = 0.0
...
chest_xray_trn_df = chest_xray_trn_df[chest_xray_trn_df.type_of_pneumonia=='pneumonia']
chest_xray_val_df = chest_xray_val_df[chest_xray_val_df.type_of_pneumonia=='pneumonia']
chest_xray_tst_df = chest_xray_tst_df[chest_xray_tst_df.type_of_pneumonia=='pneumonia']
# Showing histogram
...
rsna_df = rsna_df[rsna_df.type_of_pneumonia=='pneumonia']
...
figure1_train_df = figure1_train_df[figure1_train_df.type_of_pneumonia!='normal']
figure1_valid_df = figure1_valid_df[figure1_valid_df.type_of_pneumonia!='normal']
figure1_test_df = figure1_test_df[figure1_test_df.type_of_pneumonia!='normal']
...
# %%      
# Merge Datasets
train_df = pd.concat([rsna_train_df, covid_train_df, chest_xray_trn_df], ignore_index=True)
valid_df = pd.concat([rsna_valid_df, covid_valid_df, chest_xray_val_df], ignore_index=True)
test_df  = pd.concat([rsna_test_df, covid_test_df, chest_xray_tst_df], ignore_index=True)
...
train_df['covid'] = train_df['covid'].replace({'COVID-19': 1, 'pneumonia':0}).astype('float32')
train_df['pneumonia'] = train_df['pneumonia'].replace({'COVID-19': 0, 'pneumonia':1}).astype('float32')

valid_df['covid'] = valid_df['covid'].replace({'COVID-19': 1, 'pneumonia':0}).astype('float32')
valid_df['pneumonia'] = valid_df['pneumonia'].replace({'COVID-19': 0, 'pneumonia':1}).astype('float32')

test_df['covid'] = test_df['covid'].replace({'COVID-19': 1, 'pneumonia':0}).astype('float32')
test_df['pneumonia'] = test_df['pneumonia'].replace({'COVID-19': 0, 'pneumonia':1}).astype('float32')

# %%
columns = ['covid', 'pneumonia']

label_counts = train_df['type_of_pneumonia'].value_counts()
fig, ax1 = plt.subplots(1,1,figsize = (5, 5))
ax1.bar(np.arange(len(label_counts))+0.5, label_counts)
ax1.set_xticks(np.arange(len(label_counts))+0.5)
_ = ax1.set_xticklabels(label_counts.index, rotation = 0)
...
c0_samples_df = train_df[train_df['covid']==0]
c1_samples_df = train_df[train_df['covid']==1]

num_of_val = max(c0_samples_df.shape[0], c1_samples_df.shape[0])
res_c0_samples_df = c0_samples_df.sample(num_of_val, replace=False)
res_c1_samples_df = c1_samples_df.sample(num_of_val, replace=True)
resampled_train_df = pd.concat([res_c0_samples_df,
                                res_c1_samples_df]).sample(frac=1).reset_index(drop=True)
...
c0_samples_df = valid_df[valid_df['covid']==0]
c1_samples_df = valid_df[valid_df['covid']==1]
...
train_gen_crops = crop_generator(train_gen, IMG_SIZE, scale=(0.5, 1.0), occlusion=True)
...
# %%
#####################   Experimtents   ##################
name = 'block4_s5lr04b16br9010_0'
model_name = root_path + 'DenseNet_pneumonia_covid_' + name
...
lr_reduce = ReduceLROnPlateau(monitor=monitor, factor=0.5, min_delta=0.0001, patience=1, verbose=1)
...
# %% 
train_df.to_csv(root_path + 'train_df_pneumonia_covid_'+name)
valid_df.to_csv(root_path + 'valid_df_pneumonia_covid_'+name)
test_df.to_csv(root_path + 'test_df_pneumonia_covid_'+name)